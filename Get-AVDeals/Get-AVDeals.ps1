function Get-AVDeals ($Filter)
{
    ...
    ##Output Table
    $Table = 0..($Names.Length-1) | Select-Object @{name="Name";expression={$Names[$_]}}, @{name="Now";expression={$PricesNow[$_]}}
            #,@{name="Was";expression={$PricesWas[$_]}}
            #, @{name="Difference";expression={$DiffValue[$_]}}, @{name="% Saving";expression={$DiffPercent[$_]}}
